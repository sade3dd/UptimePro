  ...
  async alarm() {
    ...
    for (const m of serverMonitors) {
      ...
      if (now - lastCheck > 150000 && m.status !== 'down') {
        ...
        changedMonitors.push(cleanMonitor);
    ...
    // 3. 如果有任何状态变化，保存并广播
    if (hasStatusChange || dueMonitors.length > 0) {
      await this.saveToSqlite();
      ...
      if (changedMonitors.length > 0) {
        updatePayload.monitors.push(...changedMonitors);
      }
      if (dueMonitors.length > 0) {
        updatePayload.monitors.push(...dueMonitors.map(m => {
          const { history24h, ...clean } = m;
          return clean;
        }));
      }

      await this.broadcast(updatePayload);
    }

    // 4. 设定下一次闹钟
    const nextTask = allMonitors
      .filter(m => m.type !== 'server' && new Date(m.next_check).getTime() > now)
      .sort((a, b) => new Date(a.next_check).getTime() - new Date(b.next_check).getTime())[0];

    let nextAlarmTime = now + 30000; 

    if (nextTask) {
      const taskTime = new Date(nextTask.next_check).getTime();
      nextAlarmTime = Math.min(nextAlarmTime, taskTime);
    }

    nextAlarmTime = Math.max(now + 1000, nextAlarmTime);
    await this.state.storage.setAlarm(nextAlarmTime);
    
    console.log(`[MonitorEngine] Next alarm set in ${Math.round((nextAlarmTime - now) / 1000)}s`);
  }
